...
object KafkaFactory {
    fun createFhirProducer(config: KafkaConfig): Producer<UUID, ByteArray> {
    ...
    fun createFhirConsumer(config: KafkaConfig): Consumer<UUID, ByteArray> {
        val props = createCommonGcpKafkaProperties(config).also {
    ...
    private fun createCommonGcpKafkaProperties(config: KafkaConfig): Properties {
